...
o1_quality["facility_id"] = o1_staff["DMU"].values
...
# Facility IDs and location types
...
staff_map = {
    ...
    # row 24 = support worker, fully blank in this file, so excluded
    "chefs": 26,
    "allied_health": 27,
...
#log variables for SFA
eps = 1e-6
for df in [o1_clean, o2_clean, combined]:
    df["ln_y"] = np.log(df["quality_score"] + eps)
    df["ln_staff"] = np.log(df["total_staff"] + eps)
    df["ln_residents"] = np.log(df["total_residents"] + eps)
    df["ln_acuity"] = np.log(df["acuity_index"] + eps)
...
o1_clean.to_csv("o1_cleaned_for_sfa.csv", index=False)
o2_clean.to_csv("o2_cleaned_for_sfa.csv", index=False)
combined.to_csv("combined_o1_o2_for_sfa.csv", index=False)

print("\nSaved files:")
print("- o1_cleaned_for_sfa.csv")
print("- o2_cleaned_for_sfa.csv")
print("- combined_o1_o2_for_sfa.csv")